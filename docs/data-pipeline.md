...
| `merge-duplicate-fighters` | Added 2026-08-18. Fixes fighter-identity fragmentation: a scraper (`scrape-upcoming`/`backfill-missing-events`) couldn't confidently match a newly-scraped name against an existing fighter with the same exact `name` and created a second bare-bones stub instead, splitting that person's fight history/wins/losses/Elo across two DB rows. Dry-run/apply (`--dry-run` / `--apply`), same pattern as `enrich-nationality`. Canonical = whichever of the duplicate rows has more attached `Fight` rows, not whichever has the higher win/loss counter or earlier `createdAt` — those pointed at different rows for different fighters in the real data. Reassigns `Fight`/`FightStat`/`EloHistory`/`Ranking`/`UserFavorite` rows, sums `wins`/`losses`/`draws`/`noContests`, backfills null bio fields from the stub, then deletes the stub row. **Always rerun `compute-elo` afterward** — a merged fighter's rating needs their full history walked as one identity, not split across two. First real run (2026-08-18): 7 duplicate names found and merged (Bruno Silva, Mike Davis, Michael McDonald, Victor Valenzuela, Jean Silva, Tony Johnson, Joey Gomez). |
| `backfill-fight-stats` | Added 2026-08-17. Fills `FightStat` (sig strikes, takedowns, control time, knockdowns, submission attempts) for `COMPLETED` fights that have zero stats rows — every fight `sync-results`/`backfill-missing-events` has ever created, since neither writes `FightStat` at all; only `import-data` (the CSV) ever has. Source: [Cito API](https://citoapi.com), an independent third-party MMA stats API (not UFC-affiliated). Chosen over scraping ufcstats.com directly, which runs a genuine client-side proof-of-work anti-bot challenge. Requires `CITO_API_KEY` in the environment (free tier: 500 calls/month). Hard-capped at `CITO_MAX_CALLS` (default 450, leaving headroom under the monthly cap) — stops cleanly mid-run rather than exceeding budget, and is fully resumable: rerun next month and it only ever looks at fights still missing stats. Only writes `round: 0` (fight totals) rows — round-by-round has never been populated by anything and nothing in the app reads it. First real run (2026-08-17): 147/166 fights updated, 19 unmatched (bout not yet on Cito), using 162/450 calls. Rerun (2026-08-21): 5/19 remaining fights updated (UFC 330 card), 14 still unmatched (a more recent Fight Night card not yet on Cito), using 8/450 calls. |
...
| `scrape-upcoming` | Pulls the current `/events` listing page for whatever's scheduled next, then fetches each event's own `/event/<slug>` detail page (proper "UFC 330: ..." name, venue, weight classes, prelims) - PPV/numbered events used to skip this and get listing-page-only data over an unfounded bot-mitigation concern (see file header); fixed 2026-08-21. Falls back to listing-page data only if a specific event's detail-page fetch fails. Creates `UPCOMING` events and `SCHEDULED` fights (with fighter stub creation/matching for names not yet in the roster). | Minutes (now scales with event count - every event gets its own detail-page fetch, not just Fight Nights) |
