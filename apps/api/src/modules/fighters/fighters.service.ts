...
import { computeRosterPercentiles } from "./roster-percentiles";
import { computePerformanceTags, capTags } from "./performance-profile";
...
export class FightersService {
  ...
  async getBySlug(slug: string): Promise<FighterDetailDto> {
    ...
    // Same roster-wide percentile engine the Compare page's radar chart
    // uses (see getComparePercentiles below) - a fighter not in the rated
    // population gets an empty profile, not a fabricated one.
  ...
  // in absolute terms).
  //
  // Shares the same roster-wide percentile engine getBySlug's
  // performanceProfile uses (see roster-percentiles.ts) - one bulk
  // fetch + in-memory tally computed once, not two separate copies of
  // the same query drifting apart over time.
  async getComparePercentiles(slugA: string, slugB: string): Promise<ComparePercentilesDto> {
    ...
    const rosterPercentiles = await computeRosterPercentiles();
    return {
      fighterA: rosterPercentiles.get(fighterA.id) ?? NULL_PERCENTILES,
      fighterB: rosterPercentiles.get(fighterB.id) ?? NULL_PERCENTILES,
    };
  }
}
